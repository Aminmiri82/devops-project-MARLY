...
import org.marly.mavigo.service.ConsolePrinter;
...
public class MavigoApplication implements CommandLineRunner {
	...
	// there's no "loop" yet, you have to test stuff manually, feel free to change
	// stuff in this method here in order to test stuff while developing
	public void test() {
		...
		if (journeyResponse != null && journeyResponse.journeys() != null) {
			...
			// Affichage des stop areas
			ConsolePrinter.printJourneys(journeyResponse.journeys());
		} else {
	...
	// based on the type of journey, we use diffrent modes, normal, comfort,
	// touristic
	// comfort mode changes the PRIM api request based on the comfort profile before
	// calling the API
